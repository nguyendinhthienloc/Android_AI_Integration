...
def process_image(img_path, prefix, test_dir, clean_model, det_model, rec_model, rec_yml):
    print(f"\n==============================================")
    print(f"Processing: {img_path} -> Prefix: {prefix}")
    print(f"==============================================")
    
    try:
        source_img = Image.open(img_path).convert("RGB")
        src_w, src_h = source_img.size
        source_img.save(os.path.join(test_dir, f"{prefix}_1_original.png"))
        ...
        print("Saved enhanced images and comparison grid.")
        ...
        for idx, (x, y, w, h) in enumerate(boxes):
            ...
            if line_text:
                extracted_lines.append(f"Line {idx+1} [Score: {avg_conf*100:.1f}%]: {line_text}")

        txt_path = os.path.join(test_dir, f"{prefix}_7_extracted_text.txt")
        with open(txt_path, "w", encoding="utf-8") as f:
            f.write(f"=== InkClear Extracted Text Output for {prefix} ===\n\n")
            f.write("\n".join(extracted_lines) if extracted_lines else "No text detected.")
            f.write("\n\n=== End of Extracted Text ===")
        
        print(f"SUCCESS: Saved all outputs for '{prefix}' into: {test_dir}")
    except Exception as err:
        print(f"ERROR processing '{prefix}': {err}")
        import traceback
        traceback.print_exc()
...
def main():
    ...
    clean_model = os.path.abspath("app/src/main/assets/models/neural_clean.tflite")
    det_model = os.path.abspath("app/src/main/assets/models/ocr/det/inference.onnx")
    rec_model = os.path.abspath("app/src/main/assets/models/ocr/rec/inference.onnx")
    rec_yml = os.path.abspath("app/src/main/assets/models/ocr/rec/inference.yml")

    img_0_path = os.path.abspath("test_images/img_0.png")
    if os.path.exists(img_0_path):
        process_image(img_0_path, "img_0", test_dir, clean_model, det_model, rec_model, rec_yml)
    ...
    if os.path.exists(clean_sample_path):
        process_image(clean_sample_path, "clean_note", test_dir, clean_model, det_model, rec_model, rec_yml)

    ref_path = os.path.abspath("app/src/androidTest/assets/ocr_reference.png")
    if os.path.exists(ref_path):
        process_image(ref_path, "ocr_reference", test_dir, clean_model, det_model, rec_model, rec_yml)
