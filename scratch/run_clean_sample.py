...
def run_neural_clean_demo(img_path, model_path, out_grid_path):
    ...
    print("SAVED_CLEAN_GRID:", out_grid_path)

def run_ocr_det_demo(img_path, det_model_path, out_det_path):
    ...
    for cnt in contours:
        if cv2.contourArea(cnt) > 80:
            ...
            cv2.rectangle(annotated, (x, y), (x + w, y + h), (23, 50, 92), 4)
            box_count += 1

    cv2.imwrite(out_det_path, annotated)
    print(f"SAVED_OCR_BOXES: {out_det_path} (Detected {box_count} lines)")

if __name__ == "__main__":
    img = r"C:\Users\Thien Loc\.gemini\antigravity-cli\brain\633091d0-44ac-4486-a213-48cc101a26ca\clean_handwritten_note_1785926011150.jpg"
    clean_model = r"D:\InkClear\app\src\main\assets\models\neural_clean.tflite"
    ocr_det_model = r"D:\InkClear\app\src\main\assets\models\ocr\det\inference.onnx"
    
    out_dir = r"C:\Users\Thien Loc\.gemini\antigravity-cli\brain\633091d0-44ac-4486-a213-48cc101a26ca"
    out_grid = os.path.join(out_dir, "clean_demo_presentation_grid.png")
    out_ocr = os.path.join(out_dir, "clean_demo_ocr_detection.png")

    run_neural_clean_demo(img, clean_model, out_grid)
    run_ocr_det_demo(img, ocr_det_model, out_ocr)
